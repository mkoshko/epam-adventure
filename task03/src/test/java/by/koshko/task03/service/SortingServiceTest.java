...
public class SortingServiceTest {
    ...
    private final String path = getClass()
            .getResource("/data/sortSentenceNumber.txt").getPath();
    /**
     * Path to sorted text.
     */
    private final String pathToSorted = getClass()
            .getResource("/data/sortedSentenceNumber.txt").getPath();
    ...
    /**
     * Sort paragraphs in text by sentence number and compare with expected.
     *
     * @throws ServiceException see {@link SortingService} docs.
     */
    @Test
    public void testSortBySentenceNumber() throws ServiceException {
        TextComposite text = new TextComposite();
        ParserHolder.getTextParser().parse(textToSort, text);
        SortingService.sortBySentencesNumber(text);
