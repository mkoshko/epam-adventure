 ...
 */
public class SortingServiceTest2 {
    ...
    private final String path = getClass()
            .getResource("/data/sortWordNumber.txt").getPath();
    /**
     * Path to sorted text.
     */
    private final String pathToSorted = getClass()
            .getResource("/data/sortedWordNumber.txt").getPath();
    ...
    /**
     * Sort sentences in text by words number and compare with expected.
     *
     * @throws ServiceException see {@link SortingService} docs.
     */
    @Test
    public void testSortBySentencesNumber() throws ServiceException {
        TextComposite text = new TextComposite();
        ParserHolder.getTextParser().parse(textToSort, text);
        SortingService.sortByWordsNumber(text);
